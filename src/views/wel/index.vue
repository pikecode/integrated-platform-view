        ...
        <el-col :span="24">
          <basic-container>
            <el-collapse v-model="logActiveNames" @change="handleChange">
              <el-collapse-item
                title="4.7.0.RELEASE发布，升级支持SpringBoot3.5.6，新增国际化，新增字段入库加解密"
                name="43"
              >
                <div>1.[升级]SpringBoot 至 3.5.6</div>
                <div>2.[升级]SpringCloud 至 2025.0.0</div>
                <div>3.[升级]AlibabaCloud 至 2023.0.3.3</div>
                <div>4.[升级]Nacos 至 3.1.0</div>
                <div>5.[升级]MybatisPlus 至 3.5.14</div>
                <div>6.[升级]EasyExcel 至 FastExcel 1.3.0</div>
                <div>7.[升级]JJWT 至 1.3.0</div>
                <div>8.[新增]I18n国际化消息模块</div>
                <div>9.[新增]Mybatis字段加解密与加密字段模糊查询功能</div>
                <div>10.[新增]@TenantAsync注解实现租户ID异步设置功能</div>
                <div>11.[新增]@PreAuth支持多菜单权限配置，如@PreAuth(menu = "aa,bb,cc")</div>
                <div>12.[新增]BaseService通用方法扩展</div>
                <div>13.[新增]CacheUtil创建缓存支持强制指定租户ID</div>
                <div>14.[新增]演示模式配置功能</div>
                <div>15.[新增]左侧菜单支持一级路由访问</div>
                <div>16.[新增]顶部菜单增加跳转路由字段</div>
                <div>17.[优化]JWT解析逻辑，处理高并发可能出现的线程问题</div>
                <div>18.[优化]严格模式请求头支持URL传参校验</div>
                <div>19.[优化]迁移blade-starter-social至blade-core-auth模块</div>
                <div>20.[优化]Mybatis-Plus动态数据源优化加载初始化逻辑</div>
                <div>21.[优化]Mybatis-Plus开放PaginationInnerInterceptor类自定义配置</div>
                <div>22.[优化]ExcelUtil导出方法传参优化</div>
                <div>23.[优化]Condition表达式打印日志逻辑</div>
                <div>24.[优化]单点登录日期展示改为动态设置</div>
                <div>25.[优化]数据源选择驱动类时自动赋值URL模板</div>
                <div>26.[优化]租户删除逻辑完善</div>
                <div>27.[优化]日志详情接口逻辑</div>
                <div>28.[优化]iframe路由参数读取逻辑</div>
                <div>29.[修复]腾讯OSS枚举key不统一的问题</div>
              </el-collapse-item>
              <el-collapse-item
                title="4.6.0.RELEASE发布，新增KingBase兼容支持，新增数据审计模块，新增接口防抖工具"
                name="42"
              >
                <div>1.[升级]Mybatis-Plus 至 3.5.12</div>
                <div>2.[升级]Nacos-Client 至 2.5.1</div>
                <div>3.[新增]BladeX-Biz工程mqtt示例项目</div>
                <div>4.[新增]BladeX-Biz工程rabbitmq示例项目</div>
                <div>5.[新增]Kingbase兼容支持</div>
                <div>6.[新增]Oracle高版本适配</div>
                <div>7.[新增]DataRecord数据审计模块</div>
                <div>8.[新增]基于Redis实现的接口防抖工具</div>
                <div>9.[新增]Ureport实现token鉴权逻辑</div>
                <div>10.[新增]DbDynamic动态数据源独立模块，重构租户数据库隔离模块</div>
                <div>11.[新增]jwt令牌认证增加60秒误差时间，支持服务器集群时钟不同步的问题</div>
                <div>12.[新增]认证失败时增加IP锁定逻辑</div>
                <div>13.[新增]部门与用户模块的直属主管绑定功能</div>
                <div>14.[新增]ExcelUtil表头自动校验的逻辑</div>
                <div>15.[优化]租户下拉数据源接口查询逻辑</div>
                <div>16.[优化]重置密码时优先读取参数管理的配置</div>
                <div>17.[优化]prometheus组件增加jvm参数</div>
                <div>18.[优化]第三方登录增加飞书实现</div>
                <div>19.[优化]修改oauth2认证文案</div>
                <div>20.[优化]调整 TokenArgumentResolver 至 secure 模块</div>
                <div>21.[修复]LauncherService 排序失效的问题</div>
                <div>22.[修复]OAuth2Request 获取不到 token请求头的问题</div>
                <div>23.[修复]租户批量授权不生效的问题</div>
                <div>24.[修复]路由组件名称设置逻辑，确保动态路由组件名称正确赋值</div>
                <div>25.[修复]租户下拉数据源数据更新后未刷新的问题</div>
              </el-collapse-item>
              <el-collapse-item
